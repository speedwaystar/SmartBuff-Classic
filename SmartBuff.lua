                ...
                if (unit and UnitIsPlayer(unit) and not SMARTBUFF_IsInList(unit, UnitName(unit), bs.IgnoreList)) then
                  ...
                  if (UnitExists(unit) and not UnitIsDeadOrGhost(unit) and not UnitIsCorpse(unit) and UnitIsConnected(unit) and UnitIsVisible(unit) and not UnitOnTaxi(unit) and UnitInRange(unit) == 1) then
                    ...
                    if (SMARTBUFF_Options.AdvGrpBuffCheck) then
                      ...
                      if ((bs[uc] and (uct == SMARTBUFF_HUMANOID or (uc == "DRUID" and (uct == SMARTBUFF_BEAST or uct == SMARTBUFF_ELEMENTAL))))
                        or (bs["HPET"] and uct == SMARTBUFF_BEAST and uc ~= "DRUID")
                        or (bs["DKPET"] and utc == SMARTBUFF_UNDEAD)
                        or (bs["WPET"] and (uct == SMARTBUFF_DEMON or (uc ~= "DRUID" and uct == SMARTBUFF_ELEMENTAL)) and ucf ~= SMARTBUFF_DEMONTYPE)) then
                        ret, idx, buffname = SMARTBUFF_CheckUnitBuffs(unit, buffnS, buffnG);
                      end
                    else
                      ret, idx, buffname = SMARTBUFF_CheckUnitBuffs(unit, nil, buffnG);
                    end
                ...
                if (bs.MH) then
                  ...
                  if (iId and SMARTBUFF_CanApplyWeaponBuff(buffnS, iSlot)) then
                    if (bMh) then
                      ...
                      SmartBuffTooltip:SetInventoryItem("player", iSlot);
                      for i=1,SmartBuffTooltip:NumLines()do print(_G["SmartBuffTooltipTextLeft"..i]:GetText())end
                      local ttLine = _G["SmartBuffTooltipTextLeft6"];
                      if (ttLine) then
                        local ttText = ttLine:GetText();
                        if (ttText) then
                          local t = strim(string.match(ttText, "(.-)%d"));
                          print(buffnS..", "..t);
                          if (string.find(buffnS, t)) then
                            print("Same buff");
                          end
                        end
                      end
